...
class MonitorAgent(BaseAgent):
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self._fingerprint = AlertFingerprint(window_seconds=300)#5分钟内相同的告警只触发一次
        self._detector = AnomalyDetector()#多算法异常检测器
        self._history: dict[str, list[float]] = defaultdict(list)#按照指标名存储历史数据
