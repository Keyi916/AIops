...
class RCAAgent(BaseAgent):
    ...
    def __init__(self, event_bus: EventBus, knowledge_graph=None):
        ...
        self._kg = knowledge_graph#知识图谱对象
        self._reasoner = BayesianReasoner()#贝叶斯推理引擎对象

    async def process(self, state: IncidentState) -> IncidentState:
        alert = state.alert_event#获取告警事件
        if not alert:
        ...
        #沿依赖链路追踪故障传播路径，返回的是一个服务列表
        evidence = self._collect_evidence(alert, affected_chain)
        #返回的evidence是证据链，evidence是一个列表，列表中每个元素是一个字典，字典中包含证据的类型、来源、详情、权重
        candidates = self._get_fault_candidates(alert.alert_name)
        #返回的candidates是候选根因，candidates是一个列表，列表中每个元素是一个字典，字典中包含根因的描述、证据模式、基础概率、建议修复动作
