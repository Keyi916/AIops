...
class ChangeAgent(BaseAgent):
    ...
    def __init__(
        ...
        self._scorer = RiskScorer()#风险评分模型对象
        self._audit = AuditLogger()#审计日志记录器对象
        self._auto_approve_threshold = auto_approve_threshold#自动审批阈值

    async def process(self, state: IncidentState) -> IncidentState:
        heal = state.heal_event#具体修复方案，是HealEvent对象
        if not heal:
            logger.warning("[ChangeAgent] No heal event to review")
            return state

        current_hour = datetime.utcnow().hour#当前小时
        risk_score = self._scorer.compute(#计算风险评分
            action_type=heal.action_type,
            blast_radius=heal.blast_radius,
            target_service=heal.target_service,
            hour=current_hour,
        )
        #根据风险评分和修复方案，决定是否自动审批、人工审批或拒绝
