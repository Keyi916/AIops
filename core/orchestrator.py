...
class WorkflowNode:
    ...
    def __init__(
        ...
        name: str,
        agent: BaseAgent,
        condition: Optional[Callable[[IncidentState], bool]] = None,#可选函数，输入当前的instantState，输出是否执行该节点
        max_retries: int = 3,
    ):
        self.name = name
        self.agent = agent
...
class Orchestrator:
    ...
    def __init__(self, event_bus: EventBus):
        self.event_bus = event_bus#事件总线，用于发布和订阅事件
        self._nodes: list[WorkflowNode] = []
        self._checkpoints: dict[str, IncidentState] = {}#key是incident_id，value是incidentState也就是状态的快照
    ...
    async def run(
        ...
        for node in self._nodes:
            ...
            self._save_checkpoint(state)#用instent_id作为key，保存当前的instantState作为value
