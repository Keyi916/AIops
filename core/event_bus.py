...
class InMemoryEventBus(EventBus):
    ...
    def __init__(self):
        ...
        #c存每个topic的订阅者，每个订阅者是一个元组，包含group_id和处理函数
        self._running = False
        self._event_log: list[dict[str, Any]] = []

    async def publish(self, topic: str, event: BaseModel) -> None:
        #异步方法，将事件数据转换为字典并记录日志，然后调用所有订阅者的处理函数
        event_data = event.model_dump(mode="json")#将 Pydantic 模型转换为 JSON 兼容的字典
        ...
        for group_id, handler in self._subscribers.get(topic, []):
            try:
                await handler(event_data)#调用订阅者的处理函数，传入事件数据
