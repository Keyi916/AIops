...
class Severity(str, Enum):#枚举类，用于定义告警级别
    CRITICAL = "critical"
    HIGH = "high"
    MEDIUM = "medium"
    LOW = "low"
...
class AlertStatus(str, Enum):#枚举类，用于定义告警状态  【告警状态：告警中、已解决、已抑制】
    FIRING = "firing"
    RESOLVED = "resolved"
    SUPPRESSED = "suppressed"


class AgentType(str, Enum):#枚举类，用于定义agent类型 【agent类型：监控、根因分析、故障自愈、变更审批】
    MONITOR = "monitor"
    RCA = "rca"
    HEAL = "heal"
    CHANGE = "change"
...
class EventType(str, Enum):#枚举类，用于定义事件类型
...
class HealLevel(str, Enum):#枚举类，用于定义自愈级别  【自愈级别：L0、L1、L2】
    """自愈分级策略"""
    L0_AUTO = "L0"       # 全自动，无需确认
    L1_SEMI = "L1"       # 半自动，需确认
    L2_MANUAL = "L2"     # 人工介入
...
class BaseEvent(BaseModel):#基类，用于定义事件
...
class AlertEvent(BaseEvent):#告警事件
    """监控告警 Agent 产生的告警事件"""
    ...
    threshold: Optional[float] = None  #指标名、当前值、阈值（用于计算分数）
    description: str = ""
    labels: dict[str, str] = Field(default_factory=dict)


class RCAEvent(BaseEvent):#根因分析事件
...
class HealEvent(BaseEvent):#故障自愈事件
    """故障自愈 Agent 产生的修复事件"""
...
class ChangeEvent(BaseEvent):#变更审批事件
    """变更审批 Agent 产生的审批事件"""
...
class IncidentState(BaseModel):#全局状态
    ...
    change_event: Optional[ChangeEvent] = None#各阶段事件对象
