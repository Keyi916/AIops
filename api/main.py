...
from core.orchestrator import Orchestrator #调度器
from core.knowledge_graph import create_demo_knowledge_graph, InMemoryKnowledgeGraph#知识图谱，用于推理和分析
...
async def lifespan(app: FastAPI):
    #应用生命周期管理器，在应用启动时初始化事件总线、调度器和知识图谱，在应用关闭时清理资源
...
#串行执行，每个节点执行完再执行下一个节点
    result = {
        "incident_id": state.incident_id,
        "status": state.status,
        "alert": _serialize_alert(state),#分别序列化各个事件
        "rca": _serialize_rca(state),
        "heal": _serialize_heal(state),#序列化自愈事件
        "change": _serialize_change(state),#序列化变更事件
        "workflow": orchestrator.get_workflow_status() if orchestrator else None,
    }
